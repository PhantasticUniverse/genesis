...
export function randomGenome(): LeniaGenome {
  ...
  for (let i = 0; i < numPeaks; i++) {
    peaks.push(GENOME_RANGES.b.min + Math.random() * (GENOME_RANGES.b.max - GENOME_RANGES.b.min));
  }
  peaks.sort();

  return {
    R: Math.round(GENOME_RANGES.R.min + Math.random() * (GENOME_RANGES.R.max - GENOME_RANGES.R.min)),
    T: Math.round(GENOME_RANGES.T.min + Math.random() * (GENOME_RANGES.T.max - GENOME_RANGES.T.min)),
    b: peaks,
    m: GENOME_RANGES.m.min + Math.random() * (GENOME_RANGES.m.max - GENOME_RANGES.m.min),
    s: GENOME_RANGES.s.min + Math.random() * (GENOME_RANGES.s.max - GENOME_RANGES.s.min),
 ...
 * Mutate a genome
 */
export function mutateGenome(genome: LeniaGenome, mutationRate: number = 0.1): LeniaGenome {
  const mutated = cloneGenome(genome);

  // Gaussian mutation helper
  const gaussMutate = (value: number, min: number, max: number, sigma: number): number => {
    if (Math.random() > mutationRate) return value;
    const delta = (Math.random() * 2 - 1) * sigma * (max - min);
    return Math.max(min, Math.min(max, value + delta));
  };

  mutated.R = Math.round(gaussMutate(mutated.R, GENOME_RANGES.R.min, GENOME_RANGES.R.max, 0.2));
  mutated.T = Math.round(gaussMutate(mutated.T, GENOME_RANGES.T.min, GENOME_RANGES.T.max, 0.2));
  mutated.m = gaussMutate(mutated.m, GENOME_RANGES.m.min, GENOME_RANGES.m.max, 0.15);
  mutated.s = gaussMutate(mutated.s, GENOME_RANGES.s.min, GENOME_RANGES.s.max, 0.15);

  // Mutate peaks
  for (let i = 0; i < mutated.b.length; i++) {
    mutated.b[i] = gaussMutate(mutated.b[i], GENOME_RANGES.b.min, GENOME_RANGES.b.max, 0.15);
  }
  mutated.b.sort();
  ...
  if (Math.random() < mutationRate * 0.5) {
    if (Math.random() < 0.5 && mutated.b.length < 4) {
      // Add a peak
      mutated.b.push(GENOME_RANGES.b.min + Math.random() * (GENOME_RANGES.b.max - GENOME_RANGES.b.min));
      mutated.b.sort();
 ...
 */
export function crossoverGenomes(parent1: LeniaGenome, parent2: LeniaGenome): LeniaGenome {
  const alpha = 0.3;
  ...
  const child: LeniaGenome = {
    R: Math.round(blend(parent1.R, parent2.R, GENOME_RANGES.R.min, GENOME_RANGES.R.max)),
    T: Math.round(blend(parent1.T, parent2.T, GENOME_RANGES.T.min, GENOME_RANGES.T.max)),
  ...
  for (let i = 0; i < numPeaks; i++) {
    ...
    child.b.push(blend(p1Peak, p2Peak, GENOME_RANGES.b.min, GENOME_RANGES.b.max));
  }
  child.b.sort();
