...
describe('Particle System', () => {
  ...
  describe('calculateForce', () => {
    ...
    it('returns zero force for particles at same position', () => {
      const p1: Particle = { id: 0, x: 100, y: 100, vx: 0, vy: 0, type: 0, mass: 1, active: true };
      const p2: Particle = { id: 1, x: 100, y: 100, vx: 0, vy: 0, type: 0, mass: 1, active: true };
    ...
    it('returns zero force beyond maxRange', () => {
      const p1: Particle = { id: 0, x: 0, y: 0, vx: 0, vy: 0, type: 0, mass: 1, active: true };
      const p2: Particle = { id: 1, x: 100, y: 0, vx: 0, vy: 0, type: 0, mass: 1, active: true };
      const force = calculateForce(p1, p2, attractiveRule, config);
      expect(force.fx).toBe(0);
      expect(force.fy).toBe(0);
    });
    ...
    it('attractive force points toward other particle', () => {
      const p1: Particle = { id: 0, x: 100, y: 100, vx: 0, vy: 0, type: 0, mass: 1, active: true };
      const p2: Particle = { id: 1, x: 130, y: 100, vx: 0, vy: 0, type: 0, mass: 1, active: true }; // 30 units away, past equilibrium
      const force = calculateForce(p1, p2, attractiveRule, config);
    ...
    it('repulsive force points away from other particle', () => {
      const p1: Particle = { id: 0, x: 100, y: 100, vx: 0, vy: 0, type: 0, mass: 1, active: true };
      const p2: Particle = { id: 1, x: 130, y: 100, vx: 0, vy: 0, type: 0, mass: 1, active: true };
    ...
    it('force is repulsive at close range regardless of rule', () => {
      const p1: Particle = { id: 0, x: 100, y: 100, vx: 0, vy: 0, type: 0, mass: 1, active: true };
      const p2: Particle = { id: 1, x: 110, y: 100, vx: 0, vy: 0, type: 0, mass: 1, active: true }; // 10 units, less than equilibrium
      const force = calculateForce(p1, p2, attractiveRule, config);
      expect(force.fx).toBeLessThan(0); // Repulsive even with attractive rule
    });

    it('handles wrapping boundaries', () => {
      const wrappingConfig = { ...config, wrapBoundaries: true, gridWidth: 100, gridHeight: 100 };
      const p1: Particle = { id: 0, x: 5, y: 50, vx: 0, vy: 0, type: 0, mass: 1, active: true };
      const p2: Particle = { id: 1, x: 95, y: 50, vx: 0, vy: 0, type: 0, mass: 1, active: true };
    ...
    it('scales force by other particle mass', () => {
      const p1: Particle = { id: 0, x: 100, y: 100, vx: 0, vy: 0, type: 0, mass: 1, active: true };
      const p2Light: Particle = { id: 1, x: 130, y: 100, vx: 0, vy: 0, type: 0, mass: 1, active: true };
      const p2Heavy: Particle = { id: 2, x: 130, y: 100, vx: 0, vy: 0, type: 0, mass: 3, active: true };
      ...
      expect(Math.abs(forceHeavy.fx)).toBeCloseTo(Math.abs(forceLight.fx) * 3, 5);
    });
  });
  ...
  describe('spawnRandomParticles', () => {
    ...
    it('spawns particles near specified center', () => {
      ...
      spawnRandomParticles(state, 100, { centerX: 100, centerY: 100, spread: 10 });
